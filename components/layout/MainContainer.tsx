...
import { PostList, Posts } from "../../type";
import { useDispatch, useSelector } from "react-redux";
import { RootState } from "@/app/store/store";
import { updatePostList } from "@/app/store/slice";
...
const MainContainer = () => {
  ...
  const loadMorePost = async () => {
    ...
    const newPost = await GET_method(
      `/post/getAllPosts?pagesize=3&skip=${next}&type=2&uid=${userId}`
    );
    if (newPost.data?.length) {
      setPage(next);
      dispatch(
        updatePostList({ Posts: [...(postList.Posts || []), ...newPost.data] })
      );
    }
  };
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col gap-3 h-[86vh] min-w-[1200px] px-60 mx-2 overflow-auto">
      ...
      <div ref={ref} role="status" className="flex flex-row justify-center">
        <svg
          aria-hidden="true"
